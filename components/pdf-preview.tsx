...
export default function PDFPreview({ onBack }: PDFPreviewProps) {
  ...
  if (!currentResume || !previewVersion) {
    return (
      <div className="flex flex-col items-center justify-center h-[600px]">
        <p className="text-lg text-muted-foreground">이력서를 먼저 선택해주세요.</p>
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Save the PDF
      const fileName = `${currentResume.title}_${previewVersion.name.replace(/\s+/g, "_")}.pdf`
      pdf.save(fileName)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col bg-gray-100">
      ...
      <div className="p-6 rounded-lg flex justify-center min-h-[800px]">
        <div className="bg-white shadow-lg w-[210mm] min-h-[297mm]" ref={resumeRef}>
          <ResumeTemplate />
        </div>
      </div>
    </div>
