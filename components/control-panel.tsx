...
export default function ControlPanel() {
  const { addBlock, getCurrentVersion, isEditing } = useResumeStore();
  ...
  const handleAddBlock = (type: BlockType) => {
    if (!isEditing) {
      alert(
        "편집 모드에서만 블록을 추가할 수 있습니다. 상단의 '편집' 버튼을 클릭하세요."
      );
      return;
    }
    addBlock(type);
          ...
          {blockTypes.map((blockType) => {
            ...
            return (
              <Button
                ...
                disabled={
                  (blockType.type === "profile" && exists) || !isEditing
                }
              >
