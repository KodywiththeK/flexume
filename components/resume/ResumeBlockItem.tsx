...
const ResumeBlockItem = ({ block, index, moveBlock }: BlockItemProps) => {
  ...
  return (
    <div
      ref={ref}
      className={`mb-4 ${isDragging ? "opacity-50" : "opacity-100"} ${block.isHidden ? "opacity-50" : ""}`}
      ...
      <Card>
        <div className="flex items-center p-2 bg-muted border-b">
          <div
            ...
            title={isEditing ? "드래그하여 위치 변경" : "편집 모드에서만 이동 가능"}
          >
        ...
        <CardContent className={`p-4 ${block.isHidden ? "hidden" : ""}`}>{renderBlockContent()}</CardContent>
      </Card>
    </div>
  )
}
