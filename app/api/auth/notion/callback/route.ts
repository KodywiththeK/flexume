...
export async function GET(request: NextRequest) {
  ...
  // 에러 처리
  if (error) {
    return NextResponse.redirect(new URL(`/resume/notion?error=${error}`, request.url))
  }

  // 상태 값 검증 (CSRF 방지)
  if (!state || state !== savedState) {
    return NextResponse.redirect(new URL("/resume/notion?error=invalid_state", request.url))
  }

  if (!code) {
    return NextResponse.redirect(new URL("/resume/notion?error=no_code", request.url))
  }
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return NextResponse.redirect(new URL("/resume/notion?error=missing_credentials", request.url))
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(new URL("/resume/notion?error=token_exchange_failed", request.url))
    }
    ...
    if (!userResponse.ok) {
      ...
      return NextResponse.redirect(new URL("/resume/notion?error=user_fetch_failed", request.url))
    }

    const userData = await userResponse.json()

    // 성공 응답 생성
    const response = NextResponse.redirect(new URL("/resume/notion?notion_auth=success", request.url))
